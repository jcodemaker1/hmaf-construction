...
def allocation_guidance(code):
    return {
        "physical": "Prioritise direct onsite management. Use digital tools to support documentation, scheduling, traceability and information exchange, but retain physical presence for observation, judgement and leadership.",
        "hybrid_physical": "Use a hybrid allocation with greater emphasis on physical presence. Manage suitable information-based tasks digitally while protecting onsite involvement for verification, consequential decisions and human interaction.",
        "balanced": "Use a deliberately balanced hybrid allocation. Manage information-driven tasks digitally while retaining physical presence for verification, risk-sensitive decisions and relationship-dependent work.",
        "hybrid_digital": "Use a digitally leaning hybrid allocation. Manage routine coordination, reporting and information flow digitally, while retaining targeted physical presence at high-value site moments.",
        "digital": "A digital-first allocation may be appropriate for this activity if the digital environment remains reliable and no safety, verification, contractual or relational trigger requires physical presence.",
    }[code]
...
def management_plan(inputs, result, activity, ai_used, human_review, accountability, validation):
    ...
    if inputs["C"] >= 3:
        digital.append("Use standardised digital reporting and clear site-delegate/escalation arrangements to maintain management reach across the active project portfolio.")
    ...
    if inputs["L"] >= 4:
        onsite.append("Use face-to-face engagement for trust-building, mentoring, conflict resolution, difficult conversations or sensitive stakeholder interactions.")
    ...
    elif activity in ("Difficult conversation", "Mentoring / coaching workers or supervisors", "Conflict resolution", "Complex trade coordination", "Reading site conditions before a decision"):
        onsite.append("The selected activity had majority support for physical presence in the study; retain direct involvement where contextual or relational judgement is material.")

    readiness = {"Suitable devices / access": inputs["devices"], "Site connectivity": inputs["connectivity"], "Information quality / currency": inputs["information_quality"], "System integration": inputs["integration"]}
    ...
    return {"digital": _clean(digital), "onsite": _clean(onsite), "improve": _clean(improve), "reassess": _clean(reassess)}
