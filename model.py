...
def one_step_sensitivity(inputs: Dict[str, float], result: HMAFResult):
    ...
    def recalc(changes):
        ...
        return calculate_hmaf(
            data["I"], data["C"],
            data["devices"], data["connectivity"], data["information_quality"], data["integration"],
            data["R"], data["V"], data["L"]
        )
    ...
    for key in ("I", "C"):
        if inputs[key] < 5:
            ...
            scenarios.append({
                "label": f"If {names[key]} increased from {inputs[key]:.0f} to {inputs[key]+1:.0f}",
                "new_hmos": new.hmos,
    ...
    for key in ("R", "V", "L"):
        if inputs[key] > 1:
            new = recalc({key: inputs[key] - 1})
            scenarios.append({
                "label": f"If {names[key]} reduced from {inputs[key]:.0f} to {inputs[key]-1:.0f}",
                "new_hmos": new.hmos,
                "new_orientation": new.orientation,
                "changed_category": new.orientation_code != result.orientation_code,
            })
    ...
    readiness = {
        ...
        "information_quality": inputs["information_quality"],
    ...
    if readiness[weakest_key] < 5:
        ...
        scenarios.append({
            "label": f"If the weakest digital-readiness condition ({readiness_names[weakest_key]}) improved from {readiness[weakest_key]:.0f} to {readiness[weakest_key]+1:.0f}",
            "new_hmos": new.hmos,
            "new_orientation": new.orientation,
            "changed_category": new.orientation_code != result.orientation_code,
        })
